...
class tmsBuilder(gegede.builder.Builder):
    def configure(self, mat=None, thinbox1Dimension=None, thinbox2Dimension=None, gapPosition=None, BFieldUpLow = None, BFieldUpHigh = None, BFieldDownLow = None , BFieldDownHigh = None, rotationAngle = None,  **kwds):
    ...
    def construct(self, geom):        
        ...
        sci_Bar_pos_ortho = [geom.structure.Position('g')]*sci_bars
        sci_Bar_pla_ortho = [geom.structure.Placement('h',volume=scinBox_lv_ortho, pos=sci_Bar_pos_ortho[1])]*sci_bars
        ...
        for bar in range(sci_bars):
            ...
            ypos_ortho = -Q("0.83237m")+ bar * Q("0.03542m") 
        ...
        Mod_ri_rot = geom.structure.Rotation( 'Modrirot', '0deg','0deg',self.rotationAngle)
        Mod_left_rot = geom.structure.Rotation( 'Modleftrot', '0deg','0deg','-'+self.rotationAngle)
        ...
        #here is the first place I actually replace the right angled planes with the ortho planes
        ...
        thinModlayer_pos = [geom.structure.Position('i')]*Module_layers_thin
        thin_Modlayer_pla = [geom.structure.Placement('j',volume=Module_layer_lv1,pos=thinModlayer_pos[1])]*Module_layers_thin
        ...
        for module in range(Module_layers_thin):
            ...
            if module % 6 == 0 :
                #MAY NEED X AND Y FLIPPING POST TEST
        ...
        thickModlayer_pos = [geom.structure.Position('k')]*Module_layers_thick

        thick_Modlayer_pla = [geom.structure.Placement('l',volume=Module_layer_lv1,pos=thickModlayer_pos[1])]*Module_layers_thick
        ...
        #Add TMS to self
        #define materials if running on its own
